...
**팔레트와 자모표 데이터를 앱 소스에서 그대로 읽어 옵니다** — `src/styles.css`,
`src/lib/letters.js`. 데모에 옮겨 적어 두면 앱을 고칠 때마다 조용히 어긋나기 때문입니다.
앱을 고쳤으면 `npm run demo` 를 다시 돌리면 됩니다.

데모에 없는 것: 사진 업로드와 하라카트 복원. 둘 다 서버가 필요합니다.
대신 붙여넣은 아랍어를 브라우저 안에서 글자 단위로 쪼개 볼 수 있고,
