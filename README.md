...
키가 없어도 앱은 뜹니다. **예문 146개**(낱말 86 · 문장 60)가 칩으로 올라와 있어 사진 없이
재생·이전·다음·속도를 그대로 눌러 볼 수 있습니다. 사진 분석과 원문 재분석만 키를 필요로 합니다.

예문은 `src/lib/samples.js` 에 있고 모양이 `/api/read` 응답과 같습니다.
`kind` 로 낱말과 문장을 갈라 칩 목록을 나눕니다.
...
- **비행기 모드에서도 뜹니다** — 자모표, 숫자, 예문 192개, 부호대로 읽기는 전부 브라우저 안에서
...
자모표·숫자·예문 192개·부호대로 읽기가 전부 브라우저 안에서 도는 일이라 그렇습니다.
