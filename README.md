...
1. **앱이 아는 낱말에서 찾습니다** (`src/lib/dictionary.js`). 예문·자모표 예시에 손으로
   ...
   다시 뽑을 수 있습니다.
2. **그래도 못 찾으면 접사를 떼고 다시 찾습니다** (`src/lib/affix.js`). 정관사·전치사·
   접속사·소유 어미가 붙은 꼴은 사전의 표제형과 뼈대가 다릅니다 — `اَلْكِتَاب`(그 책)은
   사전에 없어도 정관사를 떼면 있는 `كِتَاب`이 나옵니다. `بِالْمَدْرَسَة`(학교에서)처럼
   전치사+정관사가 겹친 것도, `كِتَابِي`(내 책)처럼 소유 어미가 붙은 것도 같은 방식으로
   되짚습니다. 부정 목적격 명사 끝의 소리 없는 알리프(`فُنْدُقًا` 「호텔을」)도 같은
   자리에서 처리합니다 — 사전엔 `فُنْدُق` 만 있어도 찾습니다. 긴 접사부터 떼어 보고,
   그 자리에서 사전에 걸리면 멈춥니다. 붙이는 모음은 정확한 문법 격이 아니라 교재가
   흔히 쓰는 관례값입니다(소유 어미 앞은 담마, `ـي` 앞은 카스라, `ـني` 앞은 파트하) —
   그래서 결과가 문맥상의 정확한 격과 다를 수 있습니다.
3. **읽기가 여럿이면 고르게 합니다.** `كتب` 는 `كَتَبَ`(그가 썼다)일 수도
   `كُتُب`(책들)일 수도 있습니다. 하나를 골라 지어내지 않고 후보를 보여 줍니다.
4. **못 찾으면 모른다고 남깁니다.** 부호 없는 가운데 글자는 초성 하나만 나옵니다
...
접사를 다 떼도 **줄기 자체가 말뭉치에 없으면** 못 찾습니다. Tashkeela 는 법학·고전
문헌 위주라 `طَقْس`(날씨)·`دَرَّاجَة`(자전거)처럼 일상적인 현대 낱말이 통째로 안
실려 있는 경우가 있습니다 — 접사 처리로는 못 메우는 자리이고, 사전 자체를 늘리거나
(`tools/build-lexicon.mjs`) `/api/read` 로 문맥을 읽어야 풀립니다.
...
src/
  ...
  lib/affix.js           접사(정관사·전치사·소유 어미)를 떼고 사전 줄기로 되짚기
...
test/build-lexicon.test.js  낱말 다듬기 스크립트(toCitation) 단위 테스트
test/polish.test.js      낱말 다듬기 테스트
test/affix.test.js       접사 떼기 테스트
