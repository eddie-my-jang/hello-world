...
글자별 한글·로마자는 손으로 적지 않고 자모표의 자음별 발음표(`src/lib/letters.js` 의 `reads`)에서
만들어 넣었습니다 — 단어 30개면 글자 항목이 140개 가까이 되어 손으로는 검증이 안 됩니다.
`test/samples.test.js` 가 글자를 도로 이어붙이면 원래 단어가 나오는지 등을 검사합니다.
