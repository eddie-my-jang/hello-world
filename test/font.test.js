...
test('글꼴은 두 벌이고 필사체가 기본이다', async () => {
  ...
  assert.deepEqual(FONTS.map((f) => f[0]), ['naskh', 'sans'])
...
test('고른 것을 적어 두고 다시 읽는다', async () => {
  ...
  assert.equal(applyFont('sans'), 'sans')
  assert.equal(document.documentElement.dataset.font, 'sans')
  assert.equal(box.value, 'sans', '다음에 열 때를 위해 적어 둔다')
  assert.equal(readStored(), 'sans')
})

test('모르는 이름은 필사체로 되돌린다', async () => {
  stubDom({ stored: 'comic-sans' })
  const { applyFont, readStored } = await load()
...
test('적어 둘 수 없어도 이번 판에는 그대로 쓴다', async () => {
  ...
  assert.equal(readStored(), 'naskh')
  assert.equal(applyFont('sans'), 'sans')
  assert.equal(document.documentElement.dataset.font, 'sans', '화면에는 그래도 입혀야 한다')
})
